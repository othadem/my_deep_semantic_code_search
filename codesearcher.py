...
class CodeSearcher:
    ...
    def load_codebase(self):
        ...
        if not self.codebase:  # empty
            codes = codecs.open(self.path + self.conf['use_codebase'], encoding='utf-8').readlines()
            ...
            print("python codebase length ", len(self.codebase))
    ...
    def eval(self, model, poolsize, K, data_loader_class):
        ...
        data_loader = torch.utils.data.DataLoader(dataset=self.valid_set, batch_size=poolsize, shuffle=True, drop_last=True, num_workers=1)
    ...
    def repr_code(self, model, data_loader_class):
        ...
        data_loader = torch.utils.data.DataLoader(dataset=use_set, batch_size=1000, shuffle=False, drop_last=False, num_workers=1)
    ...
    def search(self, model, query, n_results=10):
        ...
        desc = gVar(desc)
        desc_repr = model.desc_encoding(desc)
        desc_repr = desc_repr.data.cpu().numpy()
    ...
    def search_thread(self, codes, sims, desc_repr, codevecs, i, n_results):
        ...
        print("python codebase ", len(self.codebase))
...
def parse_args():
    ...
    parser.add_argument("--language", choices=["java", "python"], default="java", help="Language to train the models on")
