...
class SkipAttention(nn.Module):
    ...
    def forward(self, x_input, input_lengths=None):
        embedded = self.embedding(x_input)  # input: [batch_sz x seq_len]  embedded: [batch_sz x seq_len x emb_sz]
        embedded = F.dropout(embedded, 0.25, self.training)

        rnn_output, (final_hidden_state, final_cell_state) = self.lstm(embedded)  # out:[b x seq x hid_sz*2](biRNN)
...
class DilatedCNN(nn.Module):
    def __init__(self, vocab_size, emb_size, hidden_size, n_layers=1):
        ...
        self.embedding = nn.Embedding(vocab_size, emb_size, padding_idx=0)
        self.convs1 = nn.ModuleList([nn.Conv2d(1, 80, (K, 100), dilation=1) for K in [1, 2, 3, 4, 5]])
...
class SeqEncoder(nn.Module):
    ...
    def forward(self, x_input, input_lengths=None):
        x_input = x_input.to(torch.int64)
